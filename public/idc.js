var idc = (function (root) {
  'use strict';

  // ---------- 扩展帮助方法 START ----------
  var helper = {};

  // 唯一标示 uuid, 主要用于 pageSessionId
  helper.uuid = function () {
    return 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx'.replace(/[xy]/g, function (c) {
      var r = Math.random() * 16 | 0 // 位或运算，相当于在十六以内向下取整
  ...
  helper.each = function (loopable, callback, self) {
    var additionalArgs = Array.prototype.slice.call(arguments, 3); // 截取第三个参数后的多余参数，在下面给回调使用

    if (loopable) {
      if (loopable.length === +loopable.length) { // 有 length 属性的可遍历对象
        for (var i = 0; i < loopable.length; i++) {
          callback.apply(self, [loopable[i], i].concat(additionalArgs));
        }
      } else { // 无 length 属性的可遍历对象
  ...
  // 初始化采集对象
  var collect = {
    deviceUrl: 'http://localhost:7000/collect/device/?',
    eventUrl: 'http://localhost:7000/collect/event/?',
    isuploadUrl: 'http://localhost:7000/collect/isupload/?',
    parmas: {
      ExtraInfo: {}
    }, // 参数
    device: {} // 设备信息
  ...
  collect.setDevice = function () {
    if (window && window.screen) {
      this.device.ScreenHeight = window.screen.height || 0;
      this.device.ScreenWidth = window.screen.width || 0;
      ...
      for (var i in _XT) {
        switch (_XT[i][0]) {
          case 'Target':
            ...
            break;
          case 'deviceUrl': // 配置自定义 deviceUrl
            if (_XT.userConfig) {
          ...
          case 'eventUrl': // 配置自定义 eventUrl
            ...
            break;
          case 'isuploadUrl': // 配置自定义 isuploadUrl
            if (_XT.userConfig) {
              collect.isuploadUrl = _XT[i][1].split('/?')[0] + '/?dcpChannelCode=' + _XT.userConfig.dcpChannelCode
            } else {
              collect.isuploadUrl = _XT[i][1];
            }
            break;
          default:
            break;
  ...
  collect.clickHandler = function (e) {
    ...
    if (that.openCollect == true
      && (that.target
        && helper.indexOf(that.target, currtagName) > -1
        || $target.getAttribute('collect') === "true")) {
    ...
    if (collect.loadEventSend) {
      if (collect.parmas && collect.parmas.CurrentTime && (new Date().getTime() - collect.parmas.CurrentTime) > 100) {
        ...
        if (typeof Promise == 'function') {
          ...
          // 全局清理 unload 自定义事件传入的参数，在发其他请求的时候不发
          collect.parmas.Event = 'unload'
          collect.parmas.CurrentTime = new Date().getTime()
          collect.current = collect.origin
          collect.setParams();
  ...
  collect.getBeforeload = function () {
    ...
    collect.parmas.CurrentTime = new Date().getTime()
    collect.setParams();
    collect.beforeload = helper.copyObj(collect.getParams())
  }
  ...
  collect.onload = function () {
    ...
    collect.parmas.CurrentTime = new Date().getTime()
    collect.setParams();
    collect.loaded = helper.copyObj(collect.getParams())
    collect.origin = document.URL
  ...
  collect.setIframe = function () {
    ...
    helper.on(window, "message", function (event) {
      console.log('监听message事件 setIframe', event)
  ...
  //采集 H5 信息,如果是混合应用，将采集到的信息发送给 app 端
  collect.saveEventInfo = function () {
    var that = this
    var _obj = that.getParams()
